...
public class Ficheros {
	...
	/**
     * @param errMsg         Mensaje de error a mostrar en caso de fallo en la lectura del archivo.
     * @param path           Ruta del fichero a leer.
     * @param deString_a_T   Función que permite convertir una cadena a un objeto de tipo T.
     *                       La cadena se corresponde con una de las líneas del fichero.
     * @param skipFirstLine  Indica si se debe omitir la primera línea del archivo (true) o no (false).
     * @return Una lista de objetos tipo T creados con la información incluida en cada una de las
     *         líneas del fichero.
     */
    public static <T> List<T> leeFichero(String errMsg, String path, Function<String, T> deString_a_T, boolean skipFirstLine) {
        List<T> res = null;
        try {
            List<String> lines = Files.lines(Paths.get(path), ENCODING)
                                      .collect(Collectors.toList());

            // Si se debe omitir la primera línea, se omite en el procesamiento
            if (skipFirstLine) {
